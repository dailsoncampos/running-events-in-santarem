...
RSpec.describe Scraper::EventScraper do
  around do |example|
    Dir.mktmpdir do |dir|
  ...
  let(:scraper) { described_class.new(event_store: event_store, logger: Logger.new(File::NULL)) }

  def event_rows
    Scraper::EventStore.new(@events_path).all
  end
  ...
  describe '#scrape_events' do
    ...
    context 'when website is not accessible' do
      before do
        stub_request(:get, Scraper::EventScraper::EVENTS_URL)
    ...
    describe 'should only save running events from Santarém' do
      ...
      context 'when mixed events are scraped' do
        it 'only saves valid running events from Santarém' do
          ...
          expect(event_rows.last.name).to eq('5th Road Race')
        end
      end
    end
  end
  ...
  describe '#extract_clax_url' do
    ...
    it 'percent-encodes accented characters so the URL is ASCII-only' do
      result_url = 'https://www.cronosantarem.com.br/resultados/g-live.html' \
                    '?f=eventos%2F2025%2F6%C2%AA-circuito-de-corrida-de-rua-dos-bancarios%2F6%C2%AA-circuito.clax'
      clax_url = scraper.send(:extract_clax_url, result_url)

      expect(clax_url).to eq(
        'https://www.cronosantarem.com.br/resultados/eventos/2025/' \
        '6%C2%AA-circuito-de-corrida-de-rua-dos-bancarios/6%C2%AA-circuito.clax'
      )
      expect { URI.parse(clax_url) }.not_to raise_error
    end
  end
end
